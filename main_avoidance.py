...
#target_position = np.array([10, 10, 0])  # Target point (x=10, y=10, z=0)
cone_angle_degrees = np.deg2rad(30)  # Cone angle in degrees
...
def enforce_cone_constraint(states, target_position, threshold=1.0):
    distance_to_target = np.linalg.norm(states[0:3] - target_position)
    return distance_to_target < threshold

def main():
    controller = MPCController(
    T_horizon, c_horizon, mass, I, dx, dy, dt_MPC, Q, R, P, u_min, u_max, apex, cone_angle_degrees, rho)
