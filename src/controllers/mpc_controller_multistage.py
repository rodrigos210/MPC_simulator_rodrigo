...
class MPCController:
    def __init__(self, time_horizon, c_horizon, mass, I, dx, dy, dt, Q, R, P, u_min, u_max, x_obstacle, radius_obstacle, rho, sigma, radius_spacecraft, entry_radius):
        ...
        for k in range(self.p_horizon):
            ...
            g1.append(obstacle_constraint)
            g1.append(obstacle_margin_constraint)
            
            g2.append(entry_constraint1)
            g2.append(entry_constraint2)
            g2.append(entry_constraint3)
            g2.append(entry_constraint4)
            g2.append(entry_constraint5)
            g2.append(obstacle_constraint)
            g2.append(obstacle_margin_constraint)
            ...
            J2 += mtimes([x_delta.T, Q, x_delta]) # State Deviation Cost 
            J2 += mtimes([U_k.T, R, U_k]) # Input Cost 
            J2 += rho * xi_obstacle[k]**2 # Obstacle Margin Constraint Cost
            J2 += sigma * (zeta_entry1[k] ** 2 + zeta_entry2[k] ** 2 + zeta_entry3[k] ** 2 + zeta_entry4[k] ** 2 + zeta_entry5[k] ** 2) 
        ...
        J2 += mtimes([(X[0:6] - (x_ref[0:6] + eta_target[0:6])).T, P[0:6,0:6], (X[0:6] - (x_ref[0:6] + eta_target[0:6]))])
        J2 += mtimes([(quat_err_ter + eta_target[6:10]).T , P[6:10,6:10], (quat_err_ter + eta_target[6:10])])
        J2 += mtimes([(X[10:13] - (x_ref[10:13] + eta_target[10:13])).T, P[10:13,10:13], (X[10:13] - (x_ref[10:13] + eta_target[10:13]))])
        ...
        # Solver Design
        nlp1 = {'x': vertcat(reshape(U, -1, 1), xi_obstacle, eta_target), 'f': J, 'p': p, 'g': vertcat(*g1)}
        nlp2 = {'x': vertcat(reshape(U, -1, 1), xi_obstacle, eta_target, zeta_entry1, zeta_entry2, zeta_entry3, zeta_entry4, zeta_entry5), 'f': J2, 'p': p, 'g': vertcat(*g2)}
        ...
        self.solver1 = nlpsol('solver', 'ipopt', nlp1, opts) # Solver Initiation with IPOPT
        self.solver2 = nlpsol('solver', 'ipopt', nlp2, opts) # Solver Initiation with IPOPT
    ...
    # Optimal Input Calculation
    def get_optimal_input_second(self, x0, x_ref, u_guess):
