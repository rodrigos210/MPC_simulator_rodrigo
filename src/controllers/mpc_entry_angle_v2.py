...
class MPCController:
    def __init__(self, time_horizon, c_horizon, mass, I, dx, dy, dt, Q, R, P, u_min, u_max, sigma, r_chaser, r_target, chaser_vector_C, target_vector_T, gamma):
        ...
        for k in range(self.p_horizon):
            ...
            obstacle_constraint = distance_to_target - 3 * effective_target_radius # Obstacle Constraint (HARD)
            obstacle_constraint_1st_condition = if_else(pos_prime_rotated[1] - fabs(pos_prime_rotated[0])>0, obstacle_constraint, 0)
            obstacle_constraint_2nd_condition = if_else(pos_prime_rotated[1] + pos_prime_rotated[0]<0, obstacle_constraint, 0)

            #g.append(sqrt(obstacle_constraint_1st_condition * obstacle_constraint_2nd_condition))
            #g.append(if_else(pos_prime_rotated[1] - fabs(pos_prime_rotated[0] <0 ),obstacle_constraint, 0 ))
            #g.append(if_else(pos_prime_rotated[1] <0 ,obstacle_constraint, 0 ))

            #g.append(if_else(pos_prime_rotated[1] > 0, dock_direction_constraint, 0))
            #g.append(dock_direction_constraint)
    ...
    def get_optimal_input(self, x0, x_ref, x_target, x_docking, u_guess):
        ...
        arg["lbg"] = [0, 0, 0, 0, 0, 0, 0, 0, 0] * self.p_horizon
        arg["ubg"] = [float('inf'),float('inf'),float('inf'),float('inf'),float('inf'),float('inf'),float('inf'),float('inf'),float('inf')] * self.p_horizon  
